...
namespace Microsoft.CoyoteActors.Runtime
{
    ...
    internal sealed class ProductionRuntime : ActorRuntime
    {
        ...
        {
            var mid = this.CreateActorId(typeof(ActorTimer));
            this.CreateActor(mid, typeof(ActorTimer), new TimerSetupEvent(info, owner, this.Configuration.TimeoutDelay));
            return this.GetActorFromId<ActorTimer>(mid);
        }
        ...
        public override int GetHashedExecutionState()
        {
            unchecked
            {
                ...
                {
                    int actorHash = 37;
                    actorHash = (actorHash * 397) + actor.GetHashedState(AbstractionLevel.Custom);
                    hash *= actorHash;
                }

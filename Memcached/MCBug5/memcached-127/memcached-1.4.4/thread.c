...
__attribute__((constructor)) static void register_coyote_threads(){
...
static volatile int worker_threads_started;
static volatile int worker_threads_stopped;
...
static void thread_libevent_process(int fd, void *arg);

/*
 * Initializes a connection queue.
 */
static void cq_init(CQ *cq) {
    FFI_pthread_mutex_init(&cq->lock);
    FFI_pthread_cond_init(&cq->cond);
    cq->head = NULL;
    cq->tail = NULL;
}
...
static CQ_ITEM *cq_pop(CQ *cq) {
    ...
    FFI_pthread_mutex_lock(&cq->lock);
    item = cq->head;
    if (NULL != item) {
        cq->head = item->next;
        if (NULL == cq->head)
    ...
    FFI_pthread_mutex_unlock(&cq->lock);
...
static void cq_push(CQ *cq, CQ_ITEM *item) {
    ...
    if (NULL == cq->tail)
        ...
        cq->tail->next = item;
    cq->tail = item;
    FFI_pthread_cond_signal(&cq->cond);
    FFI_pthread_mutex_unlock(&cq->lock);
}
...
static CQ_ITEM *cqi_new(void) {
    CQ_ITEM *item = NULL;
    FFI_pthread_mutex_lock(&cqi_freelist_lock);
    if (cqi_freelist) {
        item = cqi_freelist;
        cqi_freelist = item->next;
    }
    FFI_pthread_mutex_unlock(&cqi_freelist_lock);
...
static void create_worker(void *(*func)(void *), void *arg) {
    ...
    FFI_create_operation((long unsigned)(thread));
}
...
static void setup_thread(LIBEVENT_THREAD *me) {
    ...
    if (FFI_pthread_mutex_init(&me->stats.mutex) != 0) {
    ...
    me->conn_queue_elem = 0;
    me->stop = false;
    FFI_pthread_mutex_init(&(me->lock));
    FFI_pthread_cond_init(&(me->cond));
}
...
static void *worker_libevent(void *arg) {
    ...
    sleep(2); // prevent worker thread from calling start operation before create operation
    ...
    FFI_pthread_mutex_lock(&init_lock);
    init_count++;
    FFI_pthread_cond_signal(&init_cond);
    FFI_pthread_mutex_unlock(&init_lock);
    ...
    worker_threads_started ++;
	    ...
	    while(true){
    	    ...
    	    FFI_pthread_mutex_lock(&(me->lock));
    	    FFI_pthread_cond_wait(&(me->cond), &(me->lock));
    	    FFI_pthread_mutex_unlock(&(me->lock));

            if(me->stop == true) break;

    	    while(me->conn_queue_elem != 0){
        		thread_libevent_process(me->notify_receive_fd, (void *)me);
        		(me->conn_queue_elem)--;
    	    }
        }
...
void dispatch_conn_new(int sfd, enum conn_states init_state, int event_flags,
    ...
    FFI_pthread_cond_signal(&(thread->cond));

    FFI_schedule_next(); // Worker thread can now start working!
}
...
void stop_worker_threads(){
    ...
    worker_threads_stopped = 0;

    for (int i = 0; i < settings.num_threads; i++) {
        
        LIBEVENT_THREAD *thread = threads + i;
        thread->conn_queue_elem++;
        thread->stop = true;
    ...
    while(settings.num_threads != worker_threads_stopped){FFI_schedule_next();}
}
...
item *item_alloc(char *key, size_t nkey, int flags, rel_time_t exptime, int nbytes) {
    item *it;
    FFI_pthread_mutex_lock(&cache_lock);
    it = do_item_alloc(key, nkey, flags, exptime, nbytes);
    FFI_pthread_mutex_unlock(&cache_lock);
    return it;
}
...
void STATS_LOCK() {
    FFI_pthread_mutex_lock(&stats_lock);
}

void STATS_UNLOCK() {
    FFI_pthread_mutex_unlock(&stats_lock);
}

void threadlocal_stats_reset(void) {
    int ii, sid;
    for (ii = 0; ii < settings.num_threads; ++ii) {
        FFI_pthread_mutex_lock(&threads[ii].stats.mutex);
        ...
        FFI_pthread_mutex_unlock(&threads[ii].stats.mutex);
    }
}
...
void thread_init(int nthreads, struct event_base *main_base) {
    ...
    FFI_pthread_mutex_lazy_init(&cache_lock);
    FFI_pthread_mutex_lazy_init(&stats_lock);

    FFI_pthread_mutex_lazy_init(&init_lock);
    FFI_pthread_cond_lazy_init(&init_cond);

    FFI_pthread_mutex_lazy_init(&cqi_freelist_lock);
    cqi_freelist = NULL;
    ...
    while(nthreads != worker_threads_started){FFI_schedule_next();} // Wait for all worker threads to register themselves
    ...
    while (init_count < nthreads) {
        FFI_pthread_cond_wait(&init_cond, &init_lock);
    }
    FFI_pthread_mutex_unlock(&init_lock);
}

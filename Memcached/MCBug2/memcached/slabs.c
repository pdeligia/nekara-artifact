...
static int slab_rebalance_start(void) {
     ...
     */
    printf("Slab rebalance start scls: %d \n", slab_rebal.s_clsid);
...
static int slab_rebalance_move(void) {
    ...
    printf("Slab rebalance moving scls: %d \n", slab_rebal.s_clsid);
    ...
    if (slab_rebal.completed[offset] == 0) {
        ...
        printf("Slab rebalance move finished scls: %d \n", slab_rebal.s_clsid);
        pthread_mutex_unlock(&slabs_lock);
    }
...
static void slab_rebalance_finish(void) {
    ...
    printf("Within Slab rebalance finish scls: %d \n", slab_rebal.s_clsid);
#ifdef DEBUG_SLAB_MOVER
    ...
    assert(s_cls->slabs == (value - 1) && "Got the injected bug#3.");
#endif
    ...
    printf("Exiting Slab rebalance finish scls: %d \n", slab_rebal.s_clsid);
}

...
{
    /// <summary>
    /// Implements an asynchronous communicating actor. Inherit from this class
...
#pragma warning restore 618
        ...
        private async Task ExecuteAction(CachedHandler cachedAction)
        {
            try
            {
                ...
                else if (cachedAction.Handler is Func<ActorTask> actorTaskFunc)
                {
                    try
                    {
                        // We have no reliable stack for awaited operations.
        ...
        private MethodInfo GetActionWithName(string actionName)
        {
            ...
            if (method.GetCustomAttribute(typeof(AsyncStateActorAttribute)) != null)
            {
                this.Assert(method.ReturnType == typeof(Task) || method.ReturnType == typeof(ActorTask),

...
#pragma warning disable CA1822 // Mark members as static
        public void Start<TStateActor>(ref TStateActor stateActor)
            where TStateActor : IAsyncStateActor
        {
...
#pragma warning restore CA1822 // Mark members as static
        ...
        /// </summary>
        public void AwaitOnCompleted<TAwaiter, TStateActor>(ref TAwaiter awaiter, ref TStateActor stateActor)
            where TAwaiter : INotifyCompletion
            where TStateActor : IAsyncStateActor
        {
            IO.Debug.WriteLine("<AsyncBuilder> Await on completed from task '{0}'.", System.Threading.Tasks.Task.CurrentId);
            this.UseBuilder = true;
            this.MethodBuilder.AwaitOnCompleted(ref awaiter, ref stateActor);
        }

        /// <summary>
        /// Schedules the state actor to proceed to the next action when the specified awaiter completes.
        /// </summary>
        public void AwaitUnsafeOnCompleted<TAwaiter, TStateActor>(ref TAwaiter awaiter, ref TStateActor stateActor)
            where TAwaiter : ICriticalNotifyCompletion
            where TStateActor : IAsyncStateActor
        {
            IO.Debug.WriteLine("<AsyncBuilder> Await unsafe on completed from task '{0}'.", System.Threading.Tasks.Task.CurrentId);
            this.UseBuilder = true;
            this.MethodBuilder.AwaitUnsafeOnCompleted(ref awaiter, ref stateActor);
        }
    }

...
static int thread_counter = 0; // used to replace pthread_barrier_wait
static int thread_exit_counter = 0;

int FFI_pthread_barrier_wait(pthread_barrier_t *barrier)
{
  thread_counter++;
  while (thread_counter < nproc) {FFI_schedule_next();}
  thread_exit_counter++;
  if (thread_exit_counter == nproc) {thread_counter -= nproc; thread_exit_counter -= nproc;}
}
...
int main(int argc, char* argv[]){
  ...
  for(int i = 0; i < 1; i++){
    printf("Running iteration: %d, seed: %lu\n", i, FFI_seed());
...
void shuffle(Points *points)
{
  ...
  for (i=0;i<points->num-1;i++) {
    j=(FFI_next_integer(2147483648)%(points->num - i)) + i;
    temp = points->p[i];
...
void intshuffle(int *intarray, int length)
{
  ...
  int temp;
  for (i=0;i<length;i++) {
    j=(FFI_next_integer(2147483648)%(length - i))+i;
    temp = intarray[i];
  ...
  *kcenter = 1;
...

  {
    ...
    for(i = 1; i < points->num; i++ )  {
      bool to_open = ((float)FFI_next_integer(2147483648)/(float)INT_MAX)<(points->p[i].cost/z);
      if( to_open )  {
	(*kcenter)++;
	c.i = i;
...
#endif
{
  ...
  *feasible = (int *)malloc(numfeasible*sizeof(int));
  ...
  for(int i=k1; i<k2; i++ ) {
    w = (FFI_next_integer(2147483648)/(float)INT_MAX)*totalweight;
    //binary search
    l=0;
    r=points->num-1;
    if( accumweight[0] > w )  { 
...
public:
  ...
  size_t read( float* dest, int dim, int num ) {
    ...
    for( int i = 0; i < num && n > 0; i++ ) {
      for( int k = 0; k < dim; k++ ) {
	dest[i*dim + k] = FFI_next_integer(2147483648)/(float)INT_MAX;
      }
      n--;
      count++;
    }

...
namespace Test.Radical.Windows.Messaging
{
	...
	public class MessageBrokerTests
	{
		...
		public static void MessageBroker_broadcast_from_multiple_thread_should_not_fail()
		{
			...
			var subscriberThread1 = Microsoft.Coyote.Tasks.Task.Run(() =>
		   {
			   while (run)
			   {
				   try
				   {
					   broker.Subscribe<PocoTestMessage>(new Object(), (sender, msg) =>
					  {
						  Thread.Sleep(0);
					  });
				   }
			...
			var subscriberThread2 = Microsoft.Coyote.Tasks.Task.Run( () =>
			{
				while( run )
				{
					try
			...
			var broadcastThread1 = Microsoft.Coyote.Tasks.Task.Run( () =>
			{
				while( run )
				{
					try
					{
						broker.Broadcast( new Object(), new PocoTestMessage() );
					}
					catch( Exception e )
					{			
						failure = e;
			...
			run = false;
			Microsoft.Coyote.Tasks.Task.WaitAll(subscriberThread1, subscriberThread2, broadcastThread1);
